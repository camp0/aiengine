...
struct Stack3G
{
	...
	FlowForwarderPtr ff_ssl;
		...
		// Allocate all the Protocol objects
                ...
                udp_gprs = UDPProtocolPtr(new UDPProtocol());
		...
		// Allocate the Multiplexers
                ...
                mux_udp_gprs = MultiplexerPtr(new Multiplexer());
                mux_tcp = MultiplexerPtr(new Multiplexer());
                mux_icmp = MultiplexerPtr(new Multiplexer());

		// Allocate the flow caches and tables
		flow_table_udp_gprs = FlowManagerPtr(new FlowManager());
		...
		flow_cache_udp_gprs = FlowCachePtr(new FlowCache());
		...
		ff_udp_gprs = FlowForwarderPtr(new FlowForwarder());
